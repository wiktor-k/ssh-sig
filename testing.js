...
class Reader {
  ...
  readBytes(num) {
    let dv = new DataView(this.view.buffer, this.pos + this.view.byteOffset, num);
    this.pos += num;
  ...
  bytes() {
    return this.view.buffer.slice(this.view.byteOffset + this.pos, this.view.byteOffset + this.view.byteLength)
  }
}
...
let sig2 = {
  ...
  namespace, reserved, hash_algorithm,
  signature: {
      sig_algo,
